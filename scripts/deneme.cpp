...
// Real-time traffic sign detection & warning system with CNN in Python?
...
int main() {
    ...
    // Run on GPU (macOS not supporting GPU with CUDA)
    ...
    // Loop running as long as webcam is open and "p" is not pressed
    while (cap.isOpened()) {
        ...
        putText(output, "FPS: " + to_string(int(fps)), Point(50, 50), FONT_HERSHEY_DUPLEX, 1, Scalar(0, 255, 0), 2, false);
